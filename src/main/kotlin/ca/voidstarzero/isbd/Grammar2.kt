...
class Grammar2 : DSL() {
    ...
    val store: ParseState<ParseContext> = ParseState(Grammar2::class, ::ParseContext)
    ...
    private val parallelTitle = rule(
        object : AbstractForwarding(
            ...
            seq(
                ...
                parse.log.apply {
                    val context = store.data(parse)
                    val other = items[2] as? ParallelOtherTitleInfo?
                    parse.stack.push(
                            ParallelTitle(
                                items[0] as String,
                                items[1] as MaterialDesignation?,
                    ...
                    if (other == null) {
                        parse.stack.push(items[2]) // push it back if unused? before or after?
                    }
                    return@apply Runnable { /*context.parsedOtherInfo = false*/ }
                }
            }.get()
            ...
            parallelTitle.at_least(1)
                .collect().action_with_string { parse, items, _ ->
                    parse.log.apply {
                        val context = store.data(parse)
                        // context.parsedOtherInfo = true
                        val result = list<ParallelTitle>()
                        parse.stack.push(
                            ParallelTitleList(
                                items.filterNotNull().mapNotNull { it as? ParallelTitle }
                            )
                        )
                        items.filterNot { it is ParallelTitle }.forEach(parse.stack::push)
                        return@apply Runnable { /*context.parsedOtherInfo = false*/ }
                    }
                }.get()
        ) {}
